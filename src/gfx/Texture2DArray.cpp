...
namespace bgl::gfx
{
    Texture2DArray::Texture2DArray(ktxTexture2* texture, ktx_transcode_fmt_e targetFormat)
    {
        ...
        glTextureParameteri(m_handle, GL_TEXTURE_MIN_FILTER, baseTexture->numLevels > 1 ? GL_LINEAR_MIPMAP_LINEAR : GL_LINEAR);
        glTextureParameteri(m_handle, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
        glTextureParameteri(m_handle, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
        glTextureParameteri(m_handle, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);

        const uint32_t numLayers = std::max(1u, baseTexture->numLayers);
        glTextureStorage3D(m_handle, baseTexture->numLevels, internalFormat, baseTexture->baseWidth, baseTexture->baseHeight, numLayers);
        ...
        for (uint32_t level = 0; level < baseTexture->numLevels; ++level)
        {
            ...
            for (uint32_t layer = 0; layer < numLayers; ++layer)
            {
                ...
                if (isCompressed)
                {
                    glCompressedTextureSubImage3D(m_handle, level, 0, 0, layer, width, height, 1, internalFormat, static_cast<GLsizei>(imageSize), data);
                }
                else
                {
    ...
    Texture2DArray::Texture2DArray(std::span<const ImageLayer> layers, bool generateMipmaps)
    {
        ...
        for (uint32_t layer = 0; layer < numLayers; ++layer)
        {
            ...
            glTextureSubImage3D(m_handle, 0, 0, 0, layer, width, height, 1, format, GL_UNSIGNED_BYTE, layers[layer].data.data());
        }
