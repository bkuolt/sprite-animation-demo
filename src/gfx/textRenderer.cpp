    ...
    for (const auto &glyph : shapedText.glyphs)
    {
        ...
        for (uint32_t row = 0; row < bitmap.rows; ++row)
        {
            ...
            for (uint32_t col = 0; col < bitmap.width; ++col)
            {
                ...
                {
                    pixels[pixelIdx + 0] =
                        static_cast<uint8_t>((textColor.r * fgFactor + pixels[pixelIdx + 0] * bgFactor) / outAlpha);
                    pixels[pixelIdx + 1] =
                        static_cast<uint8_t>((textColor.g * fgFactor + pixels[pixelIdx + 1] * bgFactor) / outAlpha);
                    pixels[pixelIdx + 2] =
                        static_cast<uint8_t>((textColor.b * fgFactor + pixels[pixelIdx + 2] * bgFactor) / outAlpha);
